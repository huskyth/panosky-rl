...
class Runner(object):
    ...
    def __init__(self, config):
        ...
        if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec":
            self.policy = Policy(self.all_args, self.envs.observation_space[0], share_observation_space,
                                 self.envs.action_space[0], self.num_agents, device=self.device)
        else:
            self.policy = Policy(self.all_args, self.envs.observation_space[0], share_observation_space,
    ...
    def compute(self):
        ...
        if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec":
            ...
            next_values = self.trainer.policy.get_values(np.concatenate(self.buffer.share_obs_image[-1]),
                                                         np.concatenate(self.buffer.share_obs_linear[-1]),
                                                         np.concatenate(self.buffer.rnn_states_critic[-1]),
                                                         np.concatenate(self.buffer.masks[-1]))
